...
pers = []
#'adi', 'asi', 'bis', 'dinesh', 'rj', 'vin', 'yog', 'chi', 'ayush', 'shadab', 'anan', 'yadr', 'moh', 'ank']
pers.append('adi')
pers.append('asi')
pers.append('bis')
pers.append('dinesh')
...
for i in range(0, len(pers)):
    a = pd.read_csv('./Feat/' + 'walk_' + pers[i] + '_ft_01.csv')
    a1 = pd.read_csv('./Feat/' + 'walk_' + pers[i] + '_ft_02.csv')
    a2 = pd.read_csv('./Feat/' + 'walk_' + pers[i] + '_ft_03.csv')
    walk_df = pd.concat([walk_df, a, a1, a2], ignore_index=True)
    for j in range(0,len(a)+len(a1)+len(a2)):
        results.append(i)
    x[0] = x[0] + 1

walk_df = walk_df.assign(person = results)
...
for i in range(0,10,1):
    ...
    walk_df = shuffle(walk_df)

    train, test = train_test_split(walk_df, test_size=0.1)

    train_output = train[['person']].values
    train = train.drop(train.columns[[0, 31]], axis=1)
    test_output = test[['person']].values
